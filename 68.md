...
Kind 32144 events should have a `p` tag indicating the pubkey of the author of the event being reported, plus an `e` tag of the event being reported if reporting something other than kind 0. Client apps SHOULD include the recommended relay URL in the `p` and `e` tags. `a` tags with with other "shareable identifiers" as defined in [NIP-19](https://github.com/nostr-protocol/nips/blob/master/19.md), are recommended in order to help those reviewing moderation reports find the event in question.
...
```json
{
  ...
  "tags": [
    ...
    ["p", "<pubkey being labeled>", <"recommended relay URL>"],
