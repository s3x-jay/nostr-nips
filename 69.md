...
When a user specifies moderation labels, no matter if they use one of the structured vocabularies or an unstructured label, they may specify a recommended action that can be taken into consideration those using their labeling. These labels are:

Label | Meaning | Suggested defaults
--- | --- | ---
display | Do not warn users about or filter this conent | default for all cases of `MOD>PG`
warn-public | Warn people who do not directly follow the account before content is displayed | default if a context is specified
...
A `vocab` value of either `MOD` should be used with these labels.
