...
## Impact on relay servers

Relay servers which allow all users to read but only paid and whitelisted users to write SHOULD NOT block unpaid users from writing kind 32123 events that have `vocab` values of "MOD" or "X-MOD" if they have the event being reported on their relay. This solves an existing legal problem for relay owners where a user may be able to read an event, but they can not submit a report about the event. Users should not have to pay for the relay to report problems to the relay owner.
