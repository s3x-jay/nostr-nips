...
Reportable types are categories of posts which frequently need moderation.
...
The MOD vocabulary does not cover all cases where content will need to be moderated - only the cases that are likely to encompass the largest number of moderation reports. The X-MOD vocabulary will fill the gap of other cases that could use something better than an unstructured freeform text field.

The X-MOD vocabulary is intended to be organic. People and entities doing content moderation can implement terms as they see fit - in coordinaton with others who will use those terms. The X-MOD vocabulary is a good way to test the use of new terms before including those terms in the MOD vocabulary. It will also be useful for regional moderation issues that may never have enough use to be included in MOD.
...
```
Client apps should apply this label when MOD or X-MOD labels are not present on a post. Users can turn off their default label by including labels or using a kind 32123 event to label the event after publication.

In the example below the user has marked their feed as "X-MOD>IL-kng" in an attempt to say they're engaged in technically illegal activity to end the monarchy in the UK. They've also noted that their avatar picture is not "sensitive", but their website advocates for violence. Addtionally they've specified that they are engaged in political protest which should be taken into consideration as a mitigating context.
...
Labeling after publication follows all the same conventions as labeling at the time of publication, only it uses kind 32123…
...
Example: A bot scanning comparing files listed in NIP-94 events to a database of known CSAM...
```json
{
  "kind": 32123,
  "pubkey": "<pubkey of labeler>",
...
```
Note that other, non-moderator tags can be included in the same kind 32123 report.

Users should not use kind 32123 events to label their own profiles. It is not a use case client apps need to support.
