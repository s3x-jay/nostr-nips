...
...
```
Client apps should apply this label when MOD or X-MOD labels are not present on a post. Users can turn off their default label for a particular post by including labels in the post or using a kind 32123 event to label the event after publication.

In the example below the user has marked their feed as "X-MOD>IL-kng" in an attempt to say they're engaged in technically illegal activity to end the monarchy in the UK. They've also noted that their avatar picture is not "sensitive", but their website advocates violence. Addtionally they've specified that they are engaged in political protest which should be taken into consideration as a mitigating context.
...
```
Note that the example above shows how other, non-moderation tags can be included in the same kind 32123 report along with moderation tags.

Important: Users should not use kind 32123 events to label their own profiles. It is not a use case client apps need to support.
