...
To define how content moderation can be accomplished using NIP-32 labeling.
...
NIP-32 uses the `L` and `l` tags. The `L` tag is straightforward - with a namespace value of 'MOD' or 'X-MOD'.

As allowed by NIP-32, the namespace is appended to the label in `l` tags for optimized searches, and the 4th parameter is sometimes used. For example:
...
People who wish to report other types/contexts that have broad, global application should open a pull request to modify this NIP.
...
Otherwise, labeling after publication follows all the same conventions as labeling at the time of publication, only it uses kind 1985…
...
```json
{
  ...
  "tags": [
    ["e", "<eventid of event being labeled>", "<recommended relay URL>"],
    ["a", "<sharable identifier for event with extra metadata>"],
    ["p", "<pubkey of event being labeled>"],
    ["L", "MOD"],
    ["l", "MOD>delete", "MOD"],
    ["l", "MOD>IL-csa", "MOD", "{\"confidence\":1,\"degree\":1,\"support\":\"https://domain.com/csam-verifier?dbid=123\"}"]
...
```
Note that the example above shows how other, non-moderation tags can be included in the same kind 1985 report along with moderation tags.
